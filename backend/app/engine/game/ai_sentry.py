...
class SentryAIMixin:
    def _update_sentry(self, sentry: Sentry, floor: FloorState, floor_id: int) -> None:
        ...
        for p in self._players_on_floor(floor_id):
            ...
            if floor.grid[p.pos.y][p.pos.x] != TileType.FLOOR_WOOD:
                continue
            if not self._is_in_los(sentry.pos, p.pos, floor_id=floor_id):
                continue
            target = p
            break
